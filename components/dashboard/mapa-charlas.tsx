...
import { Building2, ChevronRight, Hand, MapPin, RotateCcw, School, Users, X } from 'lucide-react';
...
function SchoolPhoto({ schoolName, year }: { schoolName: string; year: number }) { const fallback = '/colegios/foto-referencial-colegio.png'; const [photos, setPhotos] = useState<string[]>([]); const [isOpen, setIsOpen] = useState(false); const [activePhoto, setActivePhoto] = useState(0); const baseName = `/colegios/${year}-${schoolPhotoSlug(schoolName)}`; useEffect(() => { let isCurrent = true; const candidates = Array.from({ length: 12 }, (_, index) => index === 0 ? `${baseName}.jpg` : `${baseName}-${index + 1}.jpg`); const loadPhoto = (url: string) => new Promise<string | null>((resolve) => { const image = new Image(); image.onload = () => resolve(url); image.onerror = () => resolve(null); image.src = url; }); void Promise.all(candidates.map(loadPhoto)).then((foundPhotos) => { if (isCurrent) { setPhotos(foundPhotos.filter((photo): photo is string => photo !== null)); setActivePhoto(0); } }); return () => { isCurrent = false; }; }, [baseName]); useEffect(() => { if (!isOpen) return; const closeWithEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') setIsOpen(false); }; document.addEventListener('keydown', closeWithEscape); return () => document.removeEventListener('keydown', closeWithEscape); }, [isOpen]); const displayPhotos = photos.length > 0 ? photos : [fallback]; const currentPhoto = displayPhotos[Math.min(activePhoto, displayPhotos.length - 1)]; const alt = photos.length > 0 ? `Foto de ${schoolName}, año ${year}` : `Foto referencial de ${schoolName}`; return <><button type="button" onClick={() => setIsOpen(true)} className="block w-full cursor-zoom-in focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[#004dff]" aria-haspopup="dialog" aria-label={`Ampliar foto de ${schoolName}`}><img src={currentPhoto} alt={alt} className="aspect-[4/3] w-full object-cover" /></button>{isOpen && <div role="dialog" aria-modal="true" aria-label={`Foto ampliada de ${schoolName}`} className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4" onClick={() => setIsOpen(false)}><div className="flex max-h-[92vh] max-w-full flex-col items-center" onClick={(event) => event.stopPropagation()}><div className="relative"><button type="button" onClick={() => setIsOpen(false)} className="absolute right-2 top-2 z-10 grid size-11 place-items-center rounded-full bg-white text-slate-800 shadow-lg focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-white" aria-label="Cerrar foto ampliada"><X className="size-6" /></button><img src={currentPhoto} alt={alt} className="max-h-[78vh] max-w-full rounded-2xl object-contain shadow-2xl" /></div>{displayPhotos.length > 1 && <><p className="mt-3 text-sm font-semibold text-white">Foto {activePhoto + 1} de {displayPhotos.length}</p><div className="mt-2 flex max-w-[92vw] gap-2 overflow-x-auto p-1">{displayPhotos.map((photo, index) => <button key={photo} type="button" onClick={() => setActivePhoto(index)} className={`shrink-0 overflow-hidden rounded-xl border-2 ${index === activePhoto ? 'border-[#ff7900]' : 'border-transparent'} focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-white`} aria-label={`Ver foto ${index + 1} de ${displayPhotos.length}`}><img src={photo} alt="" className="size-16 object-cover sm:size-20" /></button>)}</div></>}</div></div>}</>; }
