...
function SchoolPhoto({ schoolName }: { schoolName: string }) { const fallback = '/colegios/foto-referencial-colegio.png'; const [src, setSrc] = useState(`/colegios/${schoolPhotoSlug(schoolName)}.jpg`); const [isOpen, setIsOpen] = useState(false); const alt = `Foto de ${schoolName}`; useEffect(() => { if (!isOpen) return; const closeWithEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') setIsOpen(false); }; document.addEventListener('keydown', closeWithEscape); return () => document.removeEventListener('keydown', closeWithEscape); }, [isOpen]); return <><button type="button" onClick={() => setIsOpen(true)} className="block w-full cursor-zoom-in focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[#004dff]" aria-haspopup="dialog" aria-label={`Ampliar foto de ${schoolName}`}><img src={src} alt={alt} className="aspect-[4/3] w-full object-cover" onError={() => { if (src !== fallback) setSrc(fallback); }} /></button>{isOpen && <div role="dialog" aria-modal="true" aria-label={`Foto ampliada de ${schoolName}`} className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4" onClick={() => setIsOpen(false)}><div className="relative max-h-full max-w-full" onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => setIsOpen(false)} className="absolute right-2 top-2 z-10 grid size-11 place-items-center rounded-full bg-white text-slate-800 shadow-lg focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-white" aria-label="Cerrar foto ampliada"><X className="size-6" /></button><img src={src} alt={alt} className="max-h-[88vh] max-w-full rounded-2xl object-contain shadow-2xl" /></div></div>}</>; }
