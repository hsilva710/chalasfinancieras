...
const schoolMediaExtensions: Array<{ extension: string; kind: GalleryMedia['kind'] }> = [{ extension: 'jpg', kind: 'image' }, { extension: 'png', kind: 'image' }, { extension: 'webp', kind: 'image' }, { extension: 'mp4', kind: 'video' }];
const schoolMediaCache = new Map<string, Promise<GalleryMedia[]>>();
const schoolMediaCandidates = (baseName: string) => Array.from({ length: 12 }, (_, index) => schoolMediaExtensions.map(({ extension, kind }) => ({ src: `${baseName}${index === 0 ? '' : `-${index + 1}`}.${extension}`, kind }))).flat();
const findSchoolMedia = (baseName: string) => {
  const cached = schoolMediaCache.get(baseName);
  if (cached) return cached;
  const loadMedia = (item: GalleryMedia) => new Promise<GalleryMedia | null>((resolve) => {
    if (item.kind === 'image') { const image = new Image(); image.onload = () => resolve(item); image.onerror = () => resolve(null); image.src = item.src; return; }
    const video = document.createElement('video'); video.preload = 'metadata'; video.onloadedmetadata = () => resolve(item); video.onerror = () => resolve(null); video.src = item.src;
  });
  const media = Promise.all(schoolMediaCandidates(baseName).map(loadMedia)).then((foundMedia) => foundMedia.filter((item): item is GalleryMedia => item !== null));
  schoolMediaCache.set(baseName, media);
  return media;
};
...
function RegionDetail({ region, charlas, students, schools, selectedSchoolKey, onSelectSchool, onClose }: { region: RegionMeta; charlas: Charla[]; students: number; schools: number; selectedSchoolKey: string | null; onSelectSchool: (key: string | null) => void; onClose: () => void }) { return <div className="flex h-full flex-col"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold tracking-[0.13em] text-[#6b7681] uppercase">Región seleccionada</p><h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-800">{region.name}</h2></div><Button variant="outline" className="h-12 rounded-full border-[#ff7900] px-3 text-[#b85600]" onClick={onClose} aria-label="Cerrar detalle de región"><RotateCcw className="size-5" /><span className="hidden sm:inline">Volver</span></Button></div><div className="mt-5 grid grid-cols-3 gap-2 rounded-2xl bg-[#f7f9fa] p-3 text-center"><MiniStat value={formatNumber(charlas.length)} label="charlas" /><MiniStat value={formatNumber(students)} label="alumnos" /><MiniStat value={formatNumber(schools)} label="colegios" /></div>{charlas.length === 0 ? <div className="mt-8 rounded-2xl border border-dashed border-slate-300 p-6 text-center text-slate-600">Aún no se registran charlas en esta región.</div> : <div className="mt-5 flex-1 overflow-auto pr-1"><h3 className="mb-3 text-base font-semibold text-slate-700">Charlas registradas</h3><div className="grid gap-3">{charlas.map((charla, index) => { const schoolKey = `${charla.Año}-${charla.Lugar}-${charla.ColegioLocacion}-${index}`; const isOpen = schoolKey === selectedSchoolKey; return <article key={schoolKey} className="rounded-2xl border border-[#e1e5e9] p-4"><div className="flex items-start justify-between gap-3"><div><button type="button" onClick={() => onSelectSchool(isOpen ? null : schoolKey)} aria-expanded={isOpen} className="rounded text-left font-semibold leading-5 text-slate-800 underline-offset-4 transition-colors hover:text-[#b85600] hover:underline focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[#004dff]"><span className="inline-flex items-center gap-1.5">{charla.ColegioLocacion}<SchoolMediaMark schoolName={charla.ColegioLocacion} year={charla.Año} /></span></button><p className="mt-1 flex items-center gap-1.5 text-sm text-slate-500"><MapPin className="size-4 text-[#6b7681]" />{charla.Lugar}</p></div><span className="shrink-0 rounded-full bg-[#e2e9ff] px-2.5 py-1.5 text-sm font-semibold text-[#0038b8]">{formatNumber(charla.CantidaddeAlumnos)} alumnos</span></div>{isOpen && <SchoolPhoto schoolName={charla.ColegioLocacion} year={charla.Año} />}</article>; })}</div></div>}<div className="mt-5 flex items-center gap-2 text-sm text-slate-500"><Building2 className="size-4" />Datos cargados desde el archivo de charlas.</div></div>; }

function SchoolMediaMark({ schoolName, year }: { schoolName: string; year: number }) {
  const [hasMedia, setHasMedia] = useState(false);
  const baseName = `/colegios/${year}-${schoolPhotoSlug(schoolName)}`;
  useEffect(() => { let isCurrent = true; void findSchoolMedia(baseName).then((media) => { if (isCurrent) setHasMedia(media.length > 0); }); return () => { isCurrent = false; }; }, [baseName]);
  if (!hasMedia) return null;
  return <span className="text-base font-semibold leading-none text-[#b85600]" title="Este colegio tiene fotos o videos" aria-label="Este colegio tiene fotos o videos">ⓕ</span>;
}

function SchoolPhoto({ schoolName, year }: { schoolName: string; year: number }) { const [media, setMedia] = useState<GalleryMedia[]>([]); const [isLoading, setIsLoading] = useState(true); const [isOpen, setIsOpen] = useState(false); const [activeMedia, setActiveMedia] = useState(0); const baseName = `/colegios/${year}-${schoolPhotoSlug(schoolName)}`; useEffect(() => { let isCurrent = true; setMedia([]); setActiveMedia(0); setIsLoading(true); void findSchoolMedia(baseName).then((foundMedia) => { if (isCurrent) { setMedia(foundMedia); setIsLoading(false); } }); return () => { isCurrent = false; }; }, [baseName]); useEffect(() => { if (!isOpen) return; const closeWithEscape = (event: KeyboardEvent) => { if (event.key === 'Escape') setIsOpen(false); }; document.addEventListener('keydown', closeWithEscape); return () => document.removeEventListener('keydown', closeWithEscape); }, [isOpen]); if (isLoading || media.length === 0) return null; const currentMedia = media[Math.min(activeMedia, media.length - 1)]; const mediaLabel = currentMedia.kind === 'video' ? 'video' : 'foto'; const alt = `${mediaLabel} de ${schoolName}, año ${year}`; return <><div className="mt-3 overflow-hidden rounded-2xl border border-[#e1e5e9] bg-[#f7f9fa]"><button type="button" onClick={() => setIsOpen(true)} className="relative block w-full cursor-zoom-in focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-[#004dff]" aria-haspopup="dialog" aria-label={`Ampliar ${mediaLabel} de ${schoolName}`}>{currentMedia.kind === 'video' ? <><video src={currentMedia.src} muted playsInline preload="metadata" className="aspect-[4/3] w-full object-cover" /><span className="absolute inset-0 grid place-items-center"><span className="grid size-12 place-items-center rounded-full bg-black/60 text-white"><Play className="size-6 fill-current" /></span></span></> : <img src={currentMedia.src} alt={alt} className="aspect-[4/3] w-full object-cover" />}</button><p className="px-3 py-2 text-xs font-medium text-[#6b7681]">Toca el contenido para ampliarlo.</p></div>{isOpen && <div role="dialog" aria-modal="true" aria-label={`${mediaLabel} ampliado de ${schoolName}`} className="fixed inset-0 z-50 grid place-items-center bg-black/80 p-4" onClick={() => setIsOpen(false)}><div className="flex max-h-[92vh] max-w-full flex-col items-center" onClick={(event) => event.stopPropagation()}><div className="relative"><button type="button" onClick={() => setIsOpen(false)} className="absolute right-2 top-2 z-10 grid size-11 place-items-center rounded-full bg-white text-slate-800 shadow-lg focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-white" aria-label="Cerrar visor"><X className="size-6" /></button>{currentMedia.kind === 'video' ? <video key={currentMedia.src} src={currentMedia.src} controls autoPlay playsInline className="max-h-[78vh] max-w-full rounded-2xl shadow-2xl" /> : <img src={currentMedia.src} alt={alt} className="max-h-[78vh] max-w-full rounded-2xl object-contain shadow-2xl" />}</div>{media.length > 1 && <><p className="mt-3 text-sm font-semibold text-white">{currentMedia.kind === 'video' ? 'Video' : 'Foto'} {activeMedia + 1} de {media.length}</p><div className="mt-2 flex max-w-[92vw] gap-2 overflow-x-auto p-1">{media.map((item, index) => <button key={item.src} type="button" onClick={() => setActiveMedia(index)} className={`relative shrink-0 overflow-hidden rounded-xl border-2 ${index === activeMedia ? 'border-[#ff7900]' : 'border-transparent'} focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-white`} aria-label={`Ver ${item.kind === 'video' ? 'video' : 'foto'} ${index + 1} de ${media.length}`}>{item.kind === 'video' ? <><video src={item.src} muted playsInline preload="metadata" className="size-16 object-cover sm:size-20" /><span className="absolute inset-0 grid place-items-center bg-black/35 text-white"><Play className="size-5 fill-current" /></span></> : <img src={item.src} alt="" className="size-16 object-cover sm:size-20" />}</button>)}</div></>}</div></div>}</>; }
